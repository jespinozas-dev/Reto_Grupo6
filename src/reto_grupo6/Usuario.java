...
public class Usuario {
    ...
    public boolean añadirSaldo(double saldo) {
        if(saldo < 0){
            return false;
        }else{
            this.saldo += saldo;
    ...
    public boolean comprar(Producto producto) {
        if (saldo>=producto.getPrecio() && producto.reducirUnidades()) {
    ...
    public void devolver(Producto producto){
        comprasRealizadas.remove(producto);
        producto.añadirUnidades();
        añadirSaldo(producto.getPrecio());
    }
